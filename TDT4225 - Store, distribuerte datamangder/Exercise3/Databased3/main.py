...
class MongoMain:
    ...
    def q1(self):
        list = [self.collection_user, self.collection_activity,
    ...
    def q6(self):
        ...
        resultB = list(self.db[self.collection_activity].aggregate([
            {"$group": {"_id":
                        ...
                        "count":
                        {"$sum": {"$dateDiff": {"startDate": "$start_date_time", "endDate": "$end_date_time", "unit": "hour"}}}}},
            {"$sort": {"count": -1}},
            {"$limit": 1}
        ]))
    ...
    def q7(self):
        ...
        for trackpnt in range(len(all_matching_trackpoints)-1):
            ...
            start_location = (
                all_matching_trackpoints[trackpnt]['lat'], all_matching_trackpoints[trackpnt]['lon'])
    ...
    def q9(self):
        # Something along the lines we thought to do to solve this query
        # Unfortunately, we couldn't get it to work
        # We are aware that this is not the correct way to do this query and contains errors
        ...
        #     }},
        #     {"$unwind": "$tp"},
        #     {"$group": {"_id": {"user_id": "$user_id", "date_time": "$tp.date_time"}}},
        #     {"$sort": {"_id.user_id": 1}},
        # ]))
        ...
        # if element['_id']['user_id'] in dic:
        #     dic[element['_id']['user_id']].append(
        #         element['_id']['date_time'])
        # else:
        #     dic[element['_id']['user_id']] = [
        #         element['_id']['datetime']]

        # print(dic)

        # have_invalid_activites = {}
        # for k, v in dic.items():
        #     invalid = 0
        #     for x, y in zip(v[0::], v[1::]):
        #         difference = y - x
        #         time_difference_in_minutes = difference / timedelta(minutes=1)
        #         if time_difference_in_minutes >= 5:
        #             invalid += 1

        #     if invalid != 0:
        #         have_invalid_activites[k] = invalid

        # formatted_results = []
        # for k, v in have_invalid_activites.items():
        #     formatted_results.append((k, v))
        #     stop += 1
    ...
    def q11(self):
        result = list(self.db[self.collection_activity].aggregate([
            ...
            {"$group": {"_id": {"user_id": "$user_id",
                                "transportation_mode": "$transportation_mode"}, "count": {"$sum": 1}}},
    ...
    def view_top10_data(self):
        ...
        result = list(self.db[self.collection_activity].aggregate([
            {"$project": {"_id": "$_id", "user_id": "$user_id",
                          "transportation_mode": "$transportation_mode", "start_date_time": "$start_date_time", "end_date_time": "$end_date_time"}},
